...
(:durative-action navigate
  ...
  :condition (and (at start (robot_at ?robot ?from)) )
...
(:durative-action pick
  ...
  :duration (= ?duration 10)
  :condition (and (at start (robot_at ?robot ?location)) (at start (item_at ?item ?location)) (at start (gripper_free ?robot)) )
  :effect (and (at start (not (gripper_free ?robot))) (at end (carrying ?robot ?item)) (at end (not (item_at ?item ?location))) )
)
(:durative-action place
  :parameters (?robot - robot ?item - item ?location - location)
  :duration (= ?duration 10)
  :condition (and (at start (robot_at ?robot ?location)) (at start (carrying ?robot ?item)) )
  :effect (and (at start (not (carrying ?robot ?item))) (at end (gripper_free ?robot)) (at end (item_at ?item ?location)) )
)
(:durative-action take_order
  :parameters (?robot - robot ?table - location ?kitchen - location)
  :duration (= ?duration 10)
  :condition (and (at start (robot_at ?robot ?table)) (at start (order_pending ?table)) )
